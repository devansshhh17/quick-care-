...
public class Login extends JFrame implements MouseListener, ActionListener{
	...
	public Login(){
		...
		lgnBtn.setBounds(350,399,100,30);
		lgnBtn.setBackground(Color.BLACK);
		lgnBtn.setForeground(Color.WHITE);
		ButtonStyle.keepColor(lgnBtn);
		lgnBtn.addActionListener(this);
		panel.add(lgnBtn);
		
		
		exitBtn = new JButton("Exit");
		exitBtn.setBounds(465,399,100,30);
		exitBtn.setBackground(Color.BLACK);
		exitBtn.setForeground(Color.WHITE);
		ButtonStyle.keepColor(exitBtn);
	...
	public void actionPerformed(ActionEvent ae){
		...
		if(lgnBtn.getText().equals(command));{
			...
			if(name.isEmpty() || pass.isEmpty()){
				...
				if((name.equals("devansh") && pass.equals("admin")) || (name.equals("admin-1") && pass.equals("admin-1"))){
